...
class Planetesimal:
    ...
    def embryoMass_vortex(self, pf):
        """
        Calculates the embryo mass using eq.17 from Lorek
        et al. (2022), originally from Liu et al. (2020),
        and assuming that the embryo mass is 10x the
        standard mass of a planetesimal forming via the SI
        Checks that embryo mass is not larger than the total
        mass of trapped dust in the vortex
        """
        Zfil = 0.1
        Omega = (c.G.cgs.value * self.Mstar / self.model.r**3)**(1/2)
